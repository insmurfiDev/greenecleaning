            ...
            @if ($entry->{$field['name']})
                @foreach($entry->{$field['name']} as $key => $image)
                    <div class="dz-preview" data-id="{{ $key }}" data-path="{{ $image }}">
                        <img class="dropzone-thumbnail" src={{ asset('storage/'.$image) }}>
                        <a class="dz-remove" href="javascript:void(0);" data-remove="{{ $key }}" data-path="{{ $image }}">Remove
                            file</a>
                    </div>
                @endforeach
            @endif
            ...
            var dropzone = new Dropzone(".dropzone", {
                ...
                success: function (file, status) {
                    ...
                    $.each(status.images, function (key, image_path) {
                        $('.dropzone').append('<div class="dz-preview" data-id="' + key + '" data-path="' + image_path + '"><img class="dropzone-thumbnail" src="{{ url('') }}/storage/' + image_path + '" /><a class="dz-remove" href="javascript:void(0);" data-remove="' + key + '" data-path="' + image_path + '">Remove file</a></div>');
                    });
